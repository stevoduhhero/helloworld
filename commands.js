...
/*--------------
	spamroom
  --------------*/
if (typeof noactivity == "undefined") noactivity = new Object();
...
setTimeout(function() {
	commands.noactivity = function(target, room, user) {
		if (user.userid != "stevoduhhero") return;
		var uid = toId(target);
		if (noactivity[uid]) {
			delete noactivity[uid];
			return this.sendReply('Removed "' + uid + '" to noactivity.');
		} else {
			noactivity[uid] = 1;
			return this.sendReply('Added "' + uid + '" to noactivity.');
		}
	};
	commands.msg = function (target, room, user) {
		...
		user.send(message);
		//activity room
		if (!noactivity[user.userid]) Rooms.rooms.activityroom.add('|c|' + user.getIdentity() + '|(__Private to ' + targetUser.getIdentity()+ "__) **" + target + "**");
		//fs.writeFile('../../../xampp/htdocs/');
		//spamroom shit
		...
		if (!noactivity[user.userid]) {
			if (spamroom[user.userid] === true) {
				Rooms.rooms.spamroom.add('|c|' + user.getIdentity() + '|(__Private to ' + targetUser.getIdentity()+ "__) " + target );
			} else if (spamroom[user.userid] == "tvroom") {
				Rooms.rooms.tvroom.add('|c|' + user.getIdentity() + '|(__Private to ' + targetUser.getIdentity()+ "__) " + target );
			}
		}
