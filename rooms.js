...
var GlobalRoom = (function () {
	...
	GlobalRoom.prototype.startBattle = function (p1, p2, format, p1team, p2team, options) {
		...
		if (Config.reportbattles && rooms.lobby) {
			if (Bot.config.name != p1.name && Bot.config.name != p2.name) rooms.lobby.add('|b|' + newRoom.id + '|' + p1.getIdentity() + '|' + p2.getIdentity());
		}
...
var BattleRoom = (function () {
	...
	BattleRoom.prototype.onLeave = function (user) {
		...
		/* psmmo bot battles dissapear as soon as they leave */
		var bot = false,
			players = 0;
		for (var i in this.users) {
			if (this.users[i].name == Bot.config.name) bot = true;
			players++;
			if (bot && players >= 2) break; //dont expire
		}
		if (bot && players == 1) {
			//expire room
			if (this && this.expire) this.expire();
		}
	};
