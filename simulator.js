...
var Battle = (function () {
	function Battle(id, format, rated, room) {
		...
		//THIS IS IMPORTANT AS SHIT FOR THE BOOTY-BOT BOOT BOT BITCH BOOTY BOT
		this.field = {};
		//^^^^^^^^^^^^^^^^^^^^^^^ that's it, then a line on simulator.prototype.receive
	...
	Battle.prototype.receive = function (lines) {
		...
		case 'request':
			...
			if (player) {
				...
				//this shit is important for booty bot
				this.field[player.userid] = JSON.parse(this.requests[player.userid]);
				//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^das it and the stuff at the top, beauty has a price
